...
{
    [LoggerMessage(EventId = 1, Level = LogLevel.Information, Message = "Successfully retrieved all tags.")]
    public static partial void TagsRetrieved(this ILogger logger);

    [LoggerMessage(EventId = 2, Level = LogLevel.Information, Message = "No Tags were found")]
    public static partial void TagNotFoundWhileGettingAllTags(this ILogger logger);

    [LoggerMessage(EventId = 3, Level = LogLevel.Warning, Message = "AddTagToTask failed: Tag name is empty.")]
    public static partial void AddTagToTaskLogWarningTagEmpty(this ILogger logger);

    [LoggerMessage(EventId = 4, Level = LogLevel.Warning, Message = "AddTagToTask failed: Tag name '{TagName}'")]
    public static partial void AddTagToTaskLogWarningFailed(this ILogger logger, string tagName);

    [LoggerMessage(EventId = 5, Level = LogLevel.Information, Message = "Tag '{TagName}' successfully added to task {TaskId}.")]
    public static partial void TagAddedToTask(this ILogger logger, string tagName, int taskId);

    [LoggerMessage(EventId = 6, Level = LogLevel.Error, Message = "An error occurred while checking if tag name '{TagName}. {Message}")]
    ...
    [LoggerMessage(EventId = 7, Level = LogLevel.Error, Message = "An unexpected error occurred in AddTagToTask: {Message}")]
    public static partial void UnexpectedErrorOccurredWhileAddingTag(this ILogger logger, string message, Exception ex);

    [LoggerMessage(EventId = 8, Level = LogLevel.Error, Message = "An unexpected error occurred while deleting tag: {Message}")]
